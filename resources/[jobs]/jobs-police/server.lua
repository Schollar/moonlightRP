  ...
  TriggerEvent('es:getPlayerFromId', source, function(user)
    local player = user.identifier
		...
		if namePolice == "Rien" then
			TriggerClientEvent("es_freeroam:notify", source, "CHAR_ANDREAS", 1, "Police station", false, "You must be a police officer")
		else
			--updatejob(player, id) --UPDATE
			user:setenService(id)
			local isService = isService(user)
			if isService == 1 then
				TriggerClientEvent("es_freeroam:notify", source, "CHAR_ANDREAS", 1, "Police station", false, "You are in service as : ".. namePolice)
				if not civil then
					ArmeSelonGrade(id_police)
				end
			else
				TriggerClientEvent("es_freeroam:notify", source, "CHAR_ANDREAS", 1, "Police station", false, "You are now out of service")
  ...
  TriggerEvent('es:getPlayerFromId', source, function(user)
    local player = user.identifier
		...
		if namePolice == "Rien" then
			...
			local isService = isService(user)
			if isService == 1 then
				TriggerClientEvent("jobspolice:notif", source, "~g~Here's your weapon")
				TriggerClientEvent("jobspolice:giveArmory", source, id, nil)
			else
				TriggerClientEvent("jobspolice:notif", source, "~r~You are not in service")
			end
		end
  end)
end)
