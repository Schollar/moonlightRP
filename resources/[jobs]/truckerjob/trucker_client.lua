...
local TruckingCompany = {
    [0] = {["name"] = "Delivery of tank", ["x"] = 1515.51, ["y"] = -2138.76, ["z"] = 76.7922},
    [1] = {["name"] = "Delivery of container", ["x"] = 642.97, ["y"] = -3015.34, ["z"] = 7.33618},
    [2] = {["name"] = "Delivery of medicines", ["x"] = 3514.814, ["y"] = 3759.489, ["z"] = 30.0171},
    [3] = {["name"] = "Delivery of wood", ["x"] = -552.829, ["y"] = 5348.613, ["z"] = 74.754}
}
-- SPAWN POS OF THE TRAILER DEPENDING OF THE TRAILER, PLACE WE ARE ON THE MAP
local TruckingTrailer = {
    [0] = {["x"] = 1413.43, ["y"] = -2274.77, ["z"] = 65.673},
    [1] = {["x"] = 608.197, ["y"] = -2987.5, ["z"] = 6.04521},
    [2] = {["x"] = 3475.414, ["y"] = 3684.578, ["z"] = 33.888},
    [3] = {["x"] = -802.788, ["y"] = 5409.524, ["z"] = 33.857}
}
...
function tick()
    ...
    if( MISSION.start == true ) then
        ...
        if( IsEntityAttached(MISSION.trailer) and text1 == false) then
            TriggerEvent("mt:missiontext", "Drive to the ~g~destination~w~ on your map.", 10000)
            SetBlipSprite(BLIP.company, 2) --invisible
            text1 = true
        elseif( not IsEntityAttached(MISSION.trailer) and text2 == false ) then
            TriggerEvent("mt:missiontext", "Attach your ~o~trailer~w~.", 15000)
            text2 = true
        end
        Wait(2000)
        ...
        if ( GetDistanceBetweenCoords(currentMission[1], currentMission[2], currentMission[3], trailerCoords ) < 25 and not IsEntityAttached(MISSION.trailer) and MISSION.toDest == false) then
            ...
            TriggerEvent("mt:missiontext", "Return your truck to the ~y~transport company", 15000)
            trailerclear()
            MISSION.toDest = true
        elseif ( GetDistanceBetweenCoords(currentMission[1], currentMission[2], currentMission[3], trailerCoords ) < 25 and IsEntityAttached(MISSION.trailer) and MISSION.toDest == false ) then
            TriggerEvent("mt:missiontext", "You have arrived. Detach your ~o~trailer~w~ with the key ~r~H~w~", 15000)
        end
        ----- BACK TO BASE
        if (IsEntityAttached(MISSION.trailer) and MISSION.toDest == true) then
            TriggerEvent("mt:missiontext", "Detach your ~o~trailer~w~ with the key ~r~H~w~", 15000)
        end
        if ( GetDistanceBetweenCoords(BLIP.truckReturn[1], BLIP.truckReturn[2], BLIP.truckReturn[3], truckCoords ) < 25 and not IsEntityAttached(MISSION.trailer) and MISSION.toDest) then
            ...
            TriggerEvent("mt:missiontext", "You have arrived. You obtained ~g~$"..distance, 10000)
            SetBlipRoute(BLIP.company, false)
            MISSION.toDest = false
...
AddEventHandler("transporter:optionMisson2", function(trailerN)
    ...
    if trailerN == 0 then
        MissionData = {                     -- TANKER
        [0] = {263.392, -2851.05, 5.99999, 5000}, --x,y,z,money
...
elseif trailerN == 1 then
    ...
    [0] = {-1658.26, 3070.84, 31.1018, 5000}, --x,y,z,money
    [1] = {1730.39, -1569.42, 112.628, 10000},
    [2] = {10.9037, 6274.51, 31.2464, 15000},
    [3] = {1253.4, 1865.69, 79.6297, 15000}
}
elseif trailerN == 2 then
    MissionData = {                      -- FRIGORIGIQUE
    [0] = {98.501, -1607.326, 29.559, 5000}, --x,y,z,money
    [1] = {294.517,-1438.098,29.804, 10000},
    [2] = {1144.44201660156,-1484.60327148438,34.6925354003906, 15000},
    [3] = {380.510467529297,-581.022399902344,28.670919418335, 15000},
    [4] = {-654.866943359375,308.018859863281,82.8393630981445, 15000},
    [5] = {1815.92749023438,3698.14306640625,33.9835662841797, 15000},
    [6] = {-931.446960449219,-318.014434814453,39.1533203125, 15000},
    [7] = {-654.866943359375,308.018859863281,82.8393630981445, 15000},
    [8] = {-238.370208740234,6332.79248046875,32.4256858825684, 15000}
}
elseif trailerN == 3 then
    MissionData = {                      -- LOG
    [0] = {1373.795, -739.679, 67.232, 5000}, --x,y,z,money
    [1] = {-493.225, -961.148, 23.681, 10000},
    [2] = {-198.747, -1090.453, 21.687, 15000},
    [3] = {257.960, 2901.558, 43.101, 20000}
}
end
...
function MISSION.spawnTrailer(spawnID)
    MISSION.trailer = CreateVehicle(MISSION.hashTrailer, TruckingTrailer[spawnID]["x"], TruckingTrailer[spawnID]["y"], TruckingTrailer[spawnID]["z"], 0.0, true, false)
    SetVehicleOnGroundProperly(MISSION.trailer)
