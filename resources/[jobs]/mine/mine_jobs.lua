...
Citizen.CreateThread(function()
  ...
  if DrawBlipTradeShow then
    ...
    SetBlipTrade(50, "Metal Processing", 2, Position.traitement.x, Position.traitement.y, Position.traitement.z)
    SetBlipTrade(277, "Sale of minerals", 2, Position.vente.x, Position.vente.y, Position.vente.z)
    SetBlipTrade(277, "Sale of diamond", 2, Position.venteDiams.x, Position.venteDiams.y, Position.venteDiams.z)
  end
  ...
  while true do
    ...
    if not IsInVehicle() then
      if distance < Position.Recolet.distance then
        ShowInfo('~b~Press ~g~E~b~ To harvest', 0)
        if IsControlJustPressed(1, 38) then
          ...
          if myjob == 4 then
            ...
            ShowMsgtime.msg = '~r~ You must be a miner !'
            ShowMsgtime.time = 150
          end
        end
      end
    ...
    if not IsInVehicle() then
        if distance < Position.traitement.distance then
          ShowInfo('~b~Press ~g~E~b~ to process the ~b~ore', 0)
          if IsControlJustPressed(1, 38) then
            TriggerServerEvent("poleemploi:getjobs")
            Wait(100)
            if myjob == 4 then
            ...
            Wait(100)
            if roche ~= 0 then
              ShowMsgtime.msg = '~g~ Treatment ~b~of ore'
              ShowMsgtime.time = 250
              TriggerEvent("vmenu:anim" ,"pickup_object", "pickup_low")
              Wait(2500)
              ShowMsgtime.msg = '~g~ + 1 ~b~Processed Rock'
              ShowMsgtime.time = 150
            ...
            elseif cuivre ~= 0 then
              ...
              TriggerEvent("vmenu:anim" ,"pickup_object", "pickup_low")
              Wait(2500)
              ShowMsgtime.msg = '~g~ + 1 ~b~Treated Copper'
              ShowMsgtime.time = 150
            ...
            elseif fer ~= 0 then
              ...
              TriggerEvent("vmenu:anim" ,"pickup_object", "pickup_low")
              Wait(2500)
              ShowMsgtime.msg = '~g~ + 1 ~b~Treated Iron'
              ShowMsgtime.time = 150
            ...
            elseif diams ~= 0 then
              ...
              TriggerEvent("vmenu:anim" ,"pickup_object", "pickup_low")
              Wait(2500)
              ShowMsgtime.msg = '~g~ + 1 ~b~Treated Diamond'
              ShowMsgtime.time = 150

              TriggerEvent("player:looseItem", 19, 1)
              TriggerEvent("player:receiveItem", 22, 1)
            else
              ShowMsgtime.msg = "~r~ You no longer have any ore !"
              ShowMsgtime.time = 150
            end
          else
            ShowMsgtime.msg = '~r~ You must be a miner !'
    ...
    if not IsInVehicle() then
        if distance < Position.vente.distance then
          ...
          if IsControlJustPressed(1, 38) then
            ...
            Wait(100)
            if diams ~= 0 then
              ShowMsgtime.msg = '~g~ Sell ~b~Diamond'
              ShowMsgtime.time = 250
              TriggerEvent("vmenu:anim" ,"pickup_object", "pickup_low")
              Wait(2500)
              ShowMsgtime.msg = '~g~ +'..Price..'$'
