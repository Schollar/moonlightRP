    ...
    while true do
          ...
          if distanceWeedFarm < Position.Recolet.distance then
             ...
             if IsControlJustPressed(1,38) then
                  ...
                  if weedcount < 30 then
                          ShowMsgtime.msg = '~g~ Pickup ~b~Cannabis'
        ...
        if not IsInVehicle() then
          if distanceWeedFarm < Position.traitement.distance then
             ShowInfo('~b~Press ~g~E~b~ to roll ~b~Cannabis', 0)
             if IsControlJustPressed(1,38) then
                  weedcount = 0
                  -- TriggerEvent("player:getQuantity", 4, function(data)
                  --      weedcount = data.count
                  ...
                  Wait(100)
                  if weedcount ~= 0 then
                          ShowMsgtime.msg = '~g~ Roll ~b~Cannabis'
                          ShowMsgtime.time = 250
                          TriggerEvent("vmenu:anim" ,"pickup_object", "pickup_low")
                          Wait(2500)
                          ShowMsgtime.msg = '~g~ + 1 ~b~Rolled Cannabis'
                          ShowMsgtime.time = 150

                          TriggerEvent("player:looseItem", 4, 1)
                          TriggerEvent("player:receiveItem", 5, 1)
                  else
                          ShowMsgtime.msg = "~r~ You do not have Cannabis !"
                          ShowMsgtime.time = 150
                  end
             end
           end
        ...
        if not IsInVehicle() then
          if distanceWeedFarm < Position.vente.distance then
             ...
             if IsControlJustPressed(1,38) then
                  ...
                  Wait(100)
                  if weedcount ~= 0 then
                          ShowMsgtime.msg = '~g~ Sell ~b~Rolled Cannabis'
                          ShowMsgtime.time = 250
                          TriggerEvent("vmenu:anim" ,"pickup_object", "pickup_low")
                          Wait(2500)
                          ...
                          ShowMsgtime.msg = "~r~ You do not have Cannabis rolled !"
                          ShowMsgtime.time = 150
                  end
             end
           end
