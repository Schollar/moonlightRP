...
AddEventHandler('menupolice:seizecash_s', function(netID)
  TriggerEvent('es:getPlayerFromId', netID, function(user)
    ...
    TriggerClientEvent("es_freeroam:notif", source, "You have entered ".. tostring(curDCash))
  end)
end)
    ...
    if (result) then
      for _, t in pairs(inv_array_illlegal) do
          if result[t.id].quantity > 0 then
            ...
            TriggerClientEvent("es_freeroam:notif", netID, "You have lost ".. tostring(result[t.id].quantity) .." "..tostring(t.name))
            TriggerClientEvent("es_freeroam:notif", source, "You have entered ".. tostring(result[t.id].quantity) .." "..tostring(t.name))
          end
      end
    end
  end)
...
AddEventHandler('menupolice:searchveh_s', function(plate)
    ...
    if (#result > 0) then
      ...
      TriggerClientEvent("menupolice:f_searchveh", source,{name="Stolen Vehicle"})
    end
end)
