...
AddEventHandler("menutel:PhoneOG", function(target, mytel)
	...
	VMenu.AddFunc(98, "Add a contact", "tel:add", {}, "Validate")
	VMenu.AddFunc(98, "Contact the police", "tel:call", {"911"}, "Calling")
	for ind, value in pairs(PHONEBOOK) do
		VMenu.AddFunc(98, value.nom .. " " .. value.prenom .. " " .. tostring(ind), "tel:call", {ind}, "Calling: " .. tostring(ind))
	end
end)
		...
		if confirmed == 1 then
			...
			elseif UpdateOnscreenKeyboard() == 1 then
				...
				if string.len(txt) > 0 then
					...
					TriggerEvent("es_freeroam:notif", "~r~ Your message is empty")
					confirmed = 0
				end
		...
		if addconfirmed == 1 then
			...
			elseif UpdateOnscreenKeyboard() == 1 then
				...
				if (string.len(txt) > 0) and (string.match(txt, '%d%d%d(-)%d%d%d%d')) then -- BEAU REGEX PATTERN EN LUA PARCE QUE C'EST PAUVRE
					...
					TriggerEvent("es_freeroam:notif", "~r~ Enter a valid number")
					addconfirmed = 0
				end
			elseif UpdateOnscreenKeyboard() == 2 then
				addconfirmed = 0
...
Citizen.CreateThread(function()
	while true do
		...
		if (showMsg == true) then
			...
			DrawAdvancedText(0.924000000000001, 0.278, 0.005, 0.0028, 0.4, "Of ~h~~b~"..fromSMS, 255, 255, 255, 255, 6, 1)
